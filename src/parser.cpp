...
void parser::parse_eof(std::string text, location loc) {
	if (outer.empty()) {
		...
		error_report(outer.top().loc, "opening delimiter is never closed");
	}
}
...
void parser::parse_symbol(std::string text, location loc) {
	...
	if (iter == ops.end()) {
		error_report(loc, "syntax error: unknown operator");
		return;
	}
...
void parser::parse_delimiter(std::string text, location loc) {
	switch (text.size() == 1? text.front(): 0) {
		case '(': case '[': case '{': {
			...
			if (iter == delims.end()) {
				error_report(loc, "unknown delimiter");
				return;
			}
			if (!expecting_term) {
		...
		case ')': case ']': case '}': {
			if (outer.empty()) {
				error_report(loc, "unexpected closing delimiter");
				return;
			}
			if (outer.top().closer != text) {
				error_report(loc, "mismatched closing delimiter");
				return;
			}
			close(loc);
		...
		default: {
			error_report(loc, "syntax error: unknown delimiter");
		}
	}
}
